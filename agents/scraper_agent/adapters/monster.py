...
class MonsterAdapter(BaseAdapter):
    ...
    async def _scrape_via_playwright(self, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            async with async_playwright() as p:
                ...
                # Navigate to Monster jobs search
                search_url = f"{self.base_url}/jobs/search?q={keyword}&where={location}"
                ...
                for element in job_elements:
                    try:
                        job = await self._extract_job_details_from_element(element)
                        if job:
                            jobs.append(job)
    ...
    async def _extract_job_details_from_element(self, element) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            title_elem = await element.query_selector('.jobTitle, .title, h2.job-title')
            title = await title_elem.inner_text() if title_elem else ""
            
            company_elem = await element.query_selector('.companyName, .company, .employer')
            company = await company_elem.inner_text() if company_elem else ""
            
            location_elem = await element.query_selector('.jobLocation, .location, .job-location')
            location = await location_elem.inner_text() if location_elem else ""
            
            # Get job URL
            link_elem = await element.query_selector('a[href*="/job/"]')
            url = ""
            source_id = ""
            if link_elem:
            ...
            salary_elem = await element.query_selector('.salary, .pay, .compensation')
            ...
            # Extract posted date
            date_elem = await element.query_selector('.postedDate, .date, .timeago')
            ...
            job_data = {
                ...
                'url': url,
                'description': "",  # Would need to fetch full description
    ...
    def _parse_salary(self, salary_text: str) -> tuple:
        ...
        # Handle ranges like "$80K-$120K" or "80,000-120,000"
        if '-' in cleaned:
            parts = cleaned.split('-')
            try:
